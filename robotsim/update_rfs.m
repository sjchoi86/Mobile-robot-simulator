...
D2R    = pi/180;
nr_rfs = robot.nr_rfs;
result = robot.rfs_dist*ones(nr_rfs, 1);
...
robot.rfs_result = result;
result_xy        = zeros(nr_rfs, 2);
valid_idx        = zeros(nr_rfs, 1);
for i = 1:nr_rfs
    rfs_deg = robot.rfs_degs(i);
    rfs_rad = rfs_deg * D2R;
    rfs_x = (result(i) + robot.r)*cos(rfs_rad);
    rfs_y = (result(i) + robot.r)*sin(rfs_rad);
    result_xy(i, :) = [rfs_x, rfs_y];
    if result(i) ~= robot.rfs_dist
        valid_idx(i) = 1;
    else
        valid_idx(i) = 0;
    end
